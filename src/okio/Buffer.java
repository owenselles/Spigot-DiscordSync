...
public final class Buffer implements BufferedSource, BufferedSink, Cloneable
{
    ...
    public long readLong() {
        ...
        final int limit = segment.limit;
        if (limit - pos < 8) {
            return (this.readInt() & 0xFFFFFFFFL) << 32 | (this.readInt() & 0xFFFFFFFFL);
        }
        final byte[] data = segment.data;
        final long v = (data[pos++] & 0xFFL) << 56 | (data[pos++] & 0xFFL) << 48 | (data[pos++] & 0xFFL) << 40 | (data[pos++] & 0xFFL) << 32 | (data[pos++] & 0xFFL) << 24 | (data[pos++] & 0xFFL) << 16 | (data[pos++] & 0xFFL) << 8 | (data[pos++] & 0xFFL);
        this.size -= 8L;
        if (pos == limit) {
            this.head = segment.pop();
            SegmentPool.recycle(segment);
    ...
    public int select(final Options options) {
        ...
        for (int i = 0, listSize = byteStrings.length; i < listSize; ++i) {
            final ByteString b = byteStrings[i];
            if (this.size >= b.size() && this.rangeEquals(s, s.pos, b, 0, b.size())) {
                try {
                    this.skip(b.size());
                    return i;
                }
    ...
    int selectPrefix(final Options options) {
        ...
        for (int i = 0, listSize = byteStrings.length; i < listSize; ++i) {
            final ByteString b = byteStrings[i];
            final int bytesLimit = (int)Math.min(this.size, b.size());

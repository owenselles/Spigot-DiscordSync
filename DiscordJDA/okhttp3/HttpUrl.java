...
public final class HttpUrl
{
    ...
    static String canonicalize(final String input, final int pos, final int limit, final String encodeSet, final boolean alreadyEncoded, final boolean strict, final boolean plusIsSpace, final boolean asciiOnly) {
        int codePoint;
        for (int i = pos; i < limit; i += Character.charCount(codePoint)) {
    ...
    static void canonicalize(final Buffer out, final String input, final int pos, final int limit, final String encodeSet, final boolean alreadyEncoded, final boolean strict, final boolean plusIsSpace, final boolean asciiOnly) {
    ...
    public static final class Builder
    {
        ...
        public Builder setPathSegment(final int index, final String pathSegment) {
            ...
            final String canonicalPathSegment = HttpUrl.canonicalize(pathSegment, 0, pathSegment.length(), " \"<>^`{}|/\\?#", false, false, false, true);
        ...
        public Builder setEncodedPathSegment(final int index, final String encodedPathSegment) {
            ...
            final String canonicalPathSegment = HttpUrl.canonicalize(encodedPathSegment, 0, encodedPathSegment.length(), " \"<>^`{}|/\\?#", true, false, false, true);
            this.encodedPathSegments.set(index, canonicalPathSegment);
            if (this.isDot(canonicalPathSegment) || this.isDotDot(canonicalPathSegment)) {
                    ...
                    switch (c) {
                        case 64: {
                            if (!hasPassword) {
                                ...
                                final String canonicalUsername = HttpUrl.canonicalize(input, pos, passwordColonOffset, " \"':;<=>@[]^`{}|/\\?#", true, false, false, true);
                                ...
                                this.encodedPassword = this.encodedPassword + "%40" + HttpUrl.canonicalize(input, pos, componentDelimiterOffset, " \"':;<=>@[]^`{}|/\\?#", true, false, false, true);
                            }
        ...
        private void push(final String input, final int pos, final int limit, final boolean addTrailingSlash, final boolean alreadyEncoded) {
            final String segment = HttpUrl.canonicalize(input, pos, limit, " \"<>^`{}|/\\?#", alreadyEncoded, false, false, true);
        ...
        private static String canonicalizeHost(final String input, final int pos, final int limit) {
            ...
            final InetAddress inetAddress = (percentDecoded.startsWith("[") && percentDecoded.endsWith("]")) ? decodeIpv6(percentDecoded, 1, percentDecoded.length() - 1) : decodeIpv6(percentDecoded, 0, percentDecoded.length());
            if (inetAddress == null) {
                return null;
            }
        ...
        private static int parsePort(final String input, final int pos, final int limit) {
            try {
                final String portString = HttpUrl.canonicalize(input, pos, limit, "", false, false, false, true);
